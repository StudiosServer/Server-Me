...
export async function handler(chatUpdate) {
  ...
  /* Creditos a Studios Server (https://wa.me/51951225018) */
      ...
      if (typeof plugin.all === 'function') {
        try {
          ...
          /* for (const [jid] of global.reportes_solicitudes.filter(([number]) => number)) {
            ...
            if (data.exists) {
              await m.reply(`[ *!* ] n\ Lo sentimos ese comando esta teniendo problemas (por favor siga insistiendo, es posible que no tenga resultados hasta que el BOT Help resuelva el problema)`.trim(), data.jid);
            }
          }*/
...
—◉ *Contacto para apelaciones:* wa.me/51951225018
╚═════════════════════╝
               `.trim();
              m.reply(messageText);
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Studios Server;;;\nFN:Studios Server\nORG: Studios Serve\nTITLE:\nitem1.TEL;waid=51951225018:+51 951 225 018\nitem1.X-ABLabel:Studios Server\nX-WA-BIZ-DESCRIPTION:Escribe Para Informarte Sobre El Servidor.\nX-WA-BIZ-NAME:Stuios Server\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, {contacts: {displayName: 'Studios Server', contacts: [{vcard}]}}, {quoted: callmsg});
...
global.dfail = (type, m, conn) => {
  ...
  // const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: '[ ⚠ ] 𝐀𝐕𝐈𝐒𝐎 - 𝐀𝐋𝐄𝐑𝐓𝐀', body: 'Studios Server', thumbnail: imagen1, sourceUrl: 'https://google.com.mx'}}}}, aa);
